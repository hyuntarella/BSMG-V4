 ...
 * 소형 프리셋 (복합 전용, 20평이하)
 *
 * 전 항목 unit='식' lump 방식. P매트릭스 m² 단가 방식과 다름.
 * 원본: data/p-value-lump-templates.csv
 * 키: 총액(원), 값: COMPLEX_BASE 순서 [mat, labor, exp][]
 */
export type SmallPresetRow = [mat: number, labor: number, exp: number]

export const SMALL_PRESETS: Record<string, SmallPresetRow[]> = {
  '3300000': [
    [30000, 70000, 0],       // 바탕정리
    [0, 0, 0],               // 바탕조정제미장
    [90000, 110000, 0],      // 하도 프라이머
    [800000, 450000, 50000], // 복합 시트
    [120000, 180000, 0],     // 쪼인트 실란트 보강포 부착
    [650000, 400000, 50000], // 노출 우레탄
    [0, 0, 0],               // 벽체 우레탄
    [140000, 160000, 0],     // 우레탄 상도
  ],
  '3800000': [
    [50000, 100000, 0],
    [0, 0, 0],
...
/** 프리셋 총액 목록 (정렬) */
export const SMALL_PRESET_KEYS = Object.keys(SMALL_PRESETS).map(Number).sort((a, b) => a - b)
