...
면적 규칙:
- 면적이 2개 나오면: 첫 번째 = area(전체 면적), 두 번째 = wallM2(벽체 면적). 절대 합산하지 마세요.
- "487헤베 87헤베" → area:487, wallM2:87 (574가 아님!)
- "벽체" 키워드가 없어도 두 번째 면적은 wallM2입니다.
...
■ 맥락 계승:
- 직전 명령이 add_item(공종 추가)이면, 다음 발화는 그 항목의 속성 설정으로 해석. 새 공종 추가가 아님!
- 예: 직전="폐기물처리 추가" → "재료비 20 노무비 10 경비 25만" → 폐기물처리의 update_item 3개
- "식", "1회", "1식", "m²" 같은 단위 표현은 공종명이 아님. 절대 add_item의 name으로 쓰지 마세요.
