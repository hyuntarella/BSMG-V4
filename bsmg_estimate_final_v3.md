# BSMG-V4 Project Truth v3

> **v1 → v3 변경사항**: Phase 3-A (엑셀 주입 엔진) 완료 반영. §6.6 엑셀·PDF 출력 섹션 신설, §8 프로젝트 구조 갱신, §10.2 페이즈 로드맵 갱신.
> 목적: 실제 repo 코드를 1순위 진실로 삼은 단일 정정 문서.
> 기준 시점: Phase 3-A 완료 직후 (2026-04-12).
...
```
견적 데이터 (useEstimate)
  ├─ save-all API → JSON + Excel(결합) + PDF(복합) + PDF(우레탄) → Supabase Storage
  └─ export API (Phase 3-A) → 공법별 단일 XLSX → 클라이언트 직접 다운로드
```
...
### 6.5 FAB 버튼 상태 (Phase 3-A)

| 버튼 | 상태 | 동작 |
|---|---|---|
| 저장 | 활성 | save-all API (기존) |
| XLSX | 활성 | export API → 공법별 XLSX 다운로드 |
| PDF | 비활성 | "준비 중" 툴팁 (차기 페이즈) |
...
- **XLSX 생성**: ✅ 완료 (Phase 3-A)
  ...
  - 동적 행 삭제/삽입 + 수식 보존
  - is_hidden 필터링, is_locked 포함
  - 특기사항: D19 머지 셀 (B19:C22 라벨, D19:R22 내용)
- **PDF 변환**: 🔜 차기 페이즈 예정 (구글드라이브 Drive API `files.export` 활용)

---

## 7. 차기 작업 진입점

### 7.1 v3 → v4 작성 시 반영 필수
- §6 출력 파이프라인 섹션을 Phase 3-B 완료 후 갱신
- PDF 변환 완료 시 FAB PDF 버튼 재활성화 반영

---

## 8. 프로젝트 구조 (Phase 3-A 신설 파일)

| 파일 | 용도 |
|---|---|
| `templates/complex.xlsx` | 복합방수 XLSX 템플릿 |
| `templates/urethane.xlsx` | 우레탄방수 XLSX 템플릿 |
| `lib/excel/generateMethodWorkbook.ts` | 공법별 단일 XLSX 주입 엔진 |
| `app/api/estimates/[id]/export/route.ts` | XLSX 다운로드 API |

변경된 파일:
| 파일 | 변경 내용 |
|---|---|
| `lib/excel/generateWorkbook.ts` | 템플릿 경로 `templates/` 로 변경 |
| `lib/estimate/fileExport.ts` | `generateMethodExcel()` 추가 |
| `components/estimate/SaveButton.tsx` | FAB XLSX 버튼 추가, PDF 비활성화 |
...
| 페이즈 | 범위 | 상태 |
|---|---|---|
| 3-B | 구글드라이브 이관 + Drive API PDF 변환 | 다음 |
|     | - JSON+XLSX+PDF 3종 저장 | |
|     | - 서비스 계정 인증 (GOOGLE_SERVICE_ACCOUNT_JSON, GDRIVE_ESTIMATES_FOLDER_ID) | |
|     | - 컷오프 마이그레이션 (기존 Supabase estimates 유지) | |
|     | - Drive API `files.export`로 xlsx→pdf 변환 | |
|     | - FAB PDF 버튼 재활성화 | |
| 3-C | /settings UI/UX 리뉴얼 + material_increase_rate 완전 제거 | 이후 |
...
- **변경 파일**:
  ...
  - 변경: `lib/excel/generateWorkbook.ts`, `lib/estimate/fileExport.ts`, `components/estimate/SaveButton.tsx`
  - 삭제: `public/templates/complex-template.xlsx`, `public/templates/urethane-template.xlsx`
- **검증 결과**:
  - 복합 8개 아이템 → 행 삭제 + 수식 재계산 ✓
  - is_hidden 필터링 ✓, is_locked 포함 ✓
  - 14개 아이템 → 행 삽입 + 수식 재계산 ✓
  - 우레탄 7개 아이템 ✓
  - 하자이행 5/3 옵션 ✓
  - TypeScript 0 errors, Lint 0 new warnings, Build 성공
- **미해결 이슈**:
  - J9 현장주소: Estimate 타입에 address 필드 없음 → site_name으로 대체
  - PDF 변환: Phase 3-B에서 구현 예정

**END OF PROJECT TRUTH v3**
